		...
		<view class="content" @touchstart="hideEmoji">
			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView">
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.id">
					...
					<view class="my" v-if="row.from===uerInfo.appuserId">
						<view class="left">
							<view v-if="row.type=='TIMTextElem'" class="bubble">
					...
					<!-- 别人发出的消息 -->
					<view class="other" v-if="row.from!==uerInfo.appuserId">
						<view class="left">
							<image :src="row.face"></image>
						</view>
						<view class="right">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.getConversationList(option.id)
		...
		methods:{
			async getConversationList(id){
				console.log(id)
				const {data:res} = await this.tim.getConversationProfile(id)
				console.log(res)
				this.msgList = res.messageList
			},
			getMsgList(){
				// 消息列表
				let list = [
